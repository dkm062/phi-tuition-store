...
$junko_productsfound = Junko_Class::junko_shortcode_products_count();
// Ensure visibility
...
<div <?php wc_product_class( $classes ); ?>>
		...
		<div class="list-col4">
			<div class="product-image">
				...
				<?php 
					...
					if(isset($junko_opt['second_image'])){
						if($junko_opt['second_image']){
				...
				<?php if ( $product->is_on_sale() ) : ?>
					<?php echo apply_filters( 'woocommerce_sale_flash', '<span class="onsale"><span class="sale-text">' . esc_html__( 'Sale', 'junko' ) . '</span></span>', $post, $product ); ?>
				<?php endif; ?>
				<!-- end sale label -->
				...
				<ul class="actions">
					...
					<?php } ?>
					<?php if ( isset($junko_opt['quickview']) && $junko_opt['quickview'] ) { ?>
						<li class="quickviewbtn">
							<a class="detail-link quickview fa fa-external-link" data-quick-id="<?php the_ID();?>" href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php echo esc_html($junko_opt['detail_link_text']);?></a>
						</li>
					<?php } ?>
				</ul>
			</div>
	...
	<div class="product-wrapper listview">
		<div class="list-col4 <?php if($junko_viewmode=='list-view'){ echo ' col-12 col-md-4';} ?>">
			<div class="product-image">
				<?php do_action( 'woocommerce_before_shop_loop_item' ); ?>
				<?php 
					echo wp_kses($product->get_image('shop_catalog', array('class'=>'primary_image')), array(
		...
		<div class="list-col8 <?php if($junko_viewmode=='list-view'){ echo ' col-12 col-md-8';} ?>">
...
<?php if ( ( ( 0 == $woocommerce_loop['loop'] % 2 || $junko_productsfound == $woocommerce_loop['loop'] ) && $woocommerce_loop['columns'] == 2 )  ) { /* for odd case: $junko_productsfound == $woocommerce_loop['loop'] */
	echo '</div>';
} ?>
<?php if ( ( ( 0 == $woocommerce_loop['loop'] % 3 || $junko_productsfound == $woocommerce_loop['loop'] ) && $woocommerce_loop['columns'] == 3 )  ) { /* for odd case: $junko_productsfound == $woocommerce_loop['loop'] */
	echo '</div>';
} ?>
<?php if ( ( ( 0 == $woocommerce_loop['loop'] % 4 || $junko_productsfound == $woocommerce_loop['loop'] ) && $woocommerce_loop['columns'] == 4 )  ) { /* for odd case: $junko_productsfound == $woocommerce_loop['loop'] */
	echo '</div>';
} ?>
<?php if ( ( ( 0 == $woocommerce_loop['loop'] % 5 || $junko_productsfound == $woocommerce_loop['loop'] ) && $woocommerce_loop['columns'] == 5 )  ) { /* for odd case: $junko_productsfound == $woocommerce_loop['loop'] */
	echo '</div>';
} ?>