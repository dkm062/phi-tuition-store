	...
	function mm_options_generator( $option, $mm_saved_value = false, $current_class = 'none' ){
		...
		switch ( $option['type'] ) {
			...
			case 'file':
				...
				$out .= mm_common::ntab(12) . '<input class="' . $clear_full_key . ' select_file_button btn btn-primary" type="button" value="' . __( 'Select Image', 'mega_main_menu' ) . '" />';
				$out .= mm_common::ntab(11) . '</span><!-- class="input-group-btn" -->';
			...
			case 'multiplier':
				...
				$out .= mm_common::ntab(9) . '<span class="btn btn-sm btn-primary multipler_add_one_more">' . __( 'Add One More', 'mega_main_menu' ) . '</span>';
				break;
			case 'wpeditor':
...
//					'editor_class' => 'mm_must_be_wpeditor',
			...
			case 'caption':
				...
				break;
			case 'collapse_start':
				$out .= mm_common::ntab(5) . '<div class="panel bootstrap clearfix ' . str_replace( array('[',']'), array('',''), $option['key'] ) . '">';
...
*/
			...
			case 'font':
				...
				if ( $option['values'] == '' || ( is_array( $option['values'] ) && in_array( 'font_family', $option['values'] ) ) ) {
					...
					$out .= mm_common::ntab(10) . '<optgroup label="' . __( 'Installed Google Fonts', 'mega_main_menu' ). '">';
					foreach ( $set_of_google_fonts as $key => $value ) {
						if ( $value['family'] == 'Inherit' ) {
							$out .= mm_common::ntab(10) . '</optgroup>';
							$out .= mm_common::ntab(10) . '<optgroup label="' . __( 'Safe Web Fonts (Recommended)', 'mega_main_menu' ). '">';
						}
			...
			case 'background_image':
				...
				foreach ( array( __( 'Keep original', 'mega_main_menu' ) => 'auto', __( 'Stretch to width', 'mega_main_menu' ) => '100% auto', __( 'Stretch to height', 'mega_main_menu' ) => 'auto 100%','cover','contain') as $key => $value ) {
			...
			case 'gradient':
				...
				$out .= mm_common::ntab(13) . __( 'Start', 'mega_main_menu' );
				...
				$out .= mm_common::ntab(12) . '</div><!-- class="col-xs-5" -->';
				$out .= mm_common::ntab(12) . '<div class="col-xs-2 mb-10 start_end">';
				$out .= mm_common::ntab(13) . __( 'End', 'mega_main_menu' );
				$out .= mm_common::ntab(12) . '</div><!-- class="col-xs-2" -->';
				$out .= mm_common::ntab(12) . '<div class="input-group input-group-sm col-xs-5 mb-10">';
				...
				foreach ( array( __( 'Vertical', 'mega_main_menu' ) => 'top', __( 'Horizontal', 'mega_main_menu' ) => 'left', __( 'Radial', 'mega_main_menu' ) => 'radial') as $key => $value ) {
				...
				$out .= mm_common::ntab(11) . __( 'Click Here to View Result', 'mega_main_menu' );
