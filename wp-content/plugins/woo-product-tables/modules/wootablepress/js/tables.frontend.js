	...
	WtbpFrontendPage.prototype.initializeTable = (function () {
		...
		$('.wtbpTableWrapper').on('click', '.add_to_cart_button.product_type_variation', function(e) {
			...
			var	selectedProduct = [],
				...
				addFields = $this.closest('table').find('input.wtbpAddDataToCart[data-product_id="' + productIdMain + '"]'),
				variation = {};
				
			if (addFields.length) {
				var addData = {};
				addFields.each(function() {
					var key = 'wtbp_' + this.name;
					addData[key] = $(this).val();
					addData['label_' + key] = $(this).attr('data-label');
				});
				product['addData'] = addData;
			}
		...
		$( document.body ).on( 'added_to_cart', function() {
			jQuery('.wtbpContentTable').each(function( ) {
				...
				var use_add_cart_styles = app.checkSettings(settings, 'use_add_cart_styles', '0');

				if ( use_add_cart_styles == '1' ) {
					var add_cart_styles = app.checkSettings(settings, 'add_cart_styles', '');

					if ( add_cart_styles.text ) {
						jQuery(this).find('.added_to_cart.wc-forward').each(function( ) {
							jQuery(this).text(add_cart_styles.text);
						});
					}

					if ( add_cart_styles.color ) {
						jQuery(this).find('.added_to_cart.wc-forward').each(function( ) {
							jQuery(this).css('background-color', add_cart_styles.color);
						});
					}
				}
			});
