(function ($, app) {
				...
				_thisObj.tableSearch = $('#wtbpSearchTable').css('width', '100%').DataTable(
					{
						...
						fnDrawCallback : function(){
							if ( jQuery('#wtbpSearchTable_wrapper .dataTables_paginate  span .woobewoo-paginate').size() > 1) {
				...
				_thisObj.tableContent = $('.wtbpContentAdmTable').DataTable(
					{
						...
						fnDrawCallback : function(){
							if ( jQuery('#wtbpContentTable_wrapper .dataTables_paginate  span .woobewoo-paginate').size() > 1) {
	...
	AdminPage.prototype.eventsAdminPage = (function () {
		...
		settingsSection.find('select').on('change wtbp-change', function () {
			...
			if(subOptions.length) {
				...
				if(!hidden) subOptions.filter('[data-main-value*="'+value+'"]').removeClass('wtbpHidden');
	...
	AdminPage.prototype.eventsProperties = (function () {
		...
		$('#chooseColumns').on('change', function (e) {
			...
			$(selectedProperty.length == 0 || selectedProperty.attr('data-enabled') == '1' ? '#wtbpAddButton' : '.wtbpProColumn[data-type=' + selectedProperty.attr('data-type') + ']').css('display', 'inline-block');
		}).trigger('change');
	...
	AdminPage.prototype.loadProductsContentTbl = (function (byCategories, searchReload) {
		var _thisObj = this.$obj,
	...
	$("body").on("click",".wtbpSortableIcon", function(){
		var dataTable = $("#wtbpContentTable").dataTable();
		var rowCount = $('input[name="settings[productids]"]').val().split(',').length;
