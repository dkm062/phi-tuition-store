								...
								<div class="woobewoo-input-group">
									<select id="chooseColumns">
										...
										foreach ($this->table_columns as $column) {
											...
											$dataPluginDisplay = '';
											if (!empty($column['plugin_data']['acf_admin_view'])) {
												$dataPluginDisplay = ' data-plugin-display="' . $column['plugin_data']['acf_admin_view'] . '"';
												$isPluginDisplay = true;

												if (!empty($orderCols[$column['slug']])) {
													$orderCols[$column['slug']]['plugin-display'] = $isPluginDisplay;
												}
											}

											if ('id' !== $slug) {
												echo 
													'<option class="' . esc_attr( ( isset($orderCols[$slug]) && $enabled ? 'wtbpHidden ' : '' ) .
							...
							<div class="wtbpPropertiesChangeNameWrapp wtbpHidden woobewoo-plugin" title="<?php esc_html_e('Column settings', 'woo-product-tables'); ?>">
								...
								<?php if ($this->is_pro) { ?>
									<div class="wtbpOptionContainer" data-plugin="acf">
										<input type="checkbox" name="acf_display_admin_view" class="wtbpNotOutline" value="1">
										<label>
											<?php esc_html_e('Display ACF in table as admin view', 'woo-product-tables'); ?>
										</label>
									</div>
									<div class="wtbpOptionContainer" data-types="text">
										<input type="checkbox" name="acf_text_input" class="wtbpNotOutline" value="0" data-one-from="hide">
										<label><?php esc_html_e('Add additional text field to cart as product meta', 'woo-product-tables'); ?></label>
									</div>
									<div class="wtbpOptionContainer" data-properties="short_description">
