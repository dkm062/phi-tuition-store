...
class WootablepressViewWtbp extends ViewWtbp {
	...
	public function getProductThumbnailLink( $id, $imgSize, $add = 'class="wtbpMainImage"' ) {
		...
		if ($postThumbnailId) {
			$link = $this->getThumbnailLinkHtml($id, $postThumbnailId, $imgSize, $add);
		}
		return $link;
	}
	 ...
	 * @return string
	 */
	public function getProductSecondThumbnailLink( $product, $imgSize, $add = 'class="wtbpMainImage"' ) {
		$link = '';

		$gallaryFirstImgId = $this->getProductFirstGalleryImageId($product);

		if ($gallaryFirstImgId) {
			$link = $this->getThumbnailLinkHtml($product->get_id(), $gallaryFirstImgId, $imgSize, $add);
		}
	 ...
	 * @return string
	 */
	public function getThumbnailLinkHtml( $postId, $imgId, $imgSize, $add ) {
		$link = '';
		...
		if (!empty($postImg) && !empty($postImgSrc[0])) {
			$link = '<a href="' . esc_url($postImgSrc[0]) . '" data-lightbox="' . esc_attr($postId) . '" ' . $add . '>' . $postImg . '</a>';
		}
						...
						if ( $isPro ) {
							if ( $secondthumbnailActive ) {
								...
								if ($isVariation && !empty($parent_image_active) ) {
									...
									$value .= $frontend ? $this->getProductSecondThumbnailLink($paretn_product, $imgSize) : $this->getProductSecondThumbnail($paretn_product, 'thumbnail', $mobileStyles);
									// all cases exept variation with image
								} else {
									$value .= $frontend ? $this->getProductSecondThumbnailLink($_product, $imgSize) : $this->getProductSecondThumbnail($_product, 'thumbnail', $mobileStyles);
								}
							...
							if ($thumbnailCartButton) {
								if ($_product->get_stock_status() == 'outofstock') {
									...
									if ($multyAddToCart) {
										$data['check_multy'] = '<input type="checkbox" class="wtbpAddMulty" value="' . esc_attr($id) . '" data-quantity="1" data-variation_id="0">';
									}
						...
						if ($showQuantity) {
							...
							if ($quantity) {
								...
								$quantityTxt = $stockQuantityText 
									? sprintf('<span class="stock-count" data-quantity="%1$d">%2$s</span> %3$s', $quantity, $quantity, $stockQuantityText)
									: sprintf('<span class="stock-count" data-quantity="%1$d">%2$s</span> item(s)', $quantity, $quantity);
							} else {
								$quantityTxt = $stockQuantityText
										? sprintf('<span class="stock-count"></span> %1$s', $stockQuantityText)
										: '<span class="stock-count"></span> item(s)';
							}
							$value .= ( $showText ? '<br>' : '' ) . '<span class="stock-item-counts' . ( $quantity ? '' : ' wtbpHidden' ) . '">' . $quantityTxt . '</span>';
						}
						if ($colored) {
							$value .= '</span>';
						}
							...
							if ($frontend) {
								...
								if ($multyAddToCart) {
									$data['check_multy'] =
										...
										'" data-quantity="1" data-variation_id="0"' .
	...
	public function generateTableHtml( $listPost, $frontend, $settings, $withHeader = true ) {
		...
		if ($frontend && $this->getTableSetting($settings, 'multiple_add_cart', false)) {
			...
			if ( $this->getTableSetting($settings, 'multiple_add_cart_position', 'first') == 'first' || 'responsive' == $mode || 'hiding' == $mode ) {
		...
		if ($withHeader) {
			...
			foreach ($columns as $key) {
				...
				$tableHeader .= '<th data-key="' . esc_attr($key) . '"' . $noSort . '>' . ( 'check_multy' == $key ? '<input type="checkbox" class="wtbpAddMultyAll">': esc_html($this->getColumnNiceName($key)) ) . '</th>';
			}
			$tableHeader .= '</tr>';
		}
		$tableBody = '';
